...
export class PortfolioSystem {
    ...
    async load() {
        ...
        if (data) {
            ...
            this.carrots = Math.max(0, Number(data.gameData?.count) || 0);
    ...
    async processTrade(type, amount, price) {
        ...
        if (type === 'buy') {
            ...
            this.carrots = Math.max(0, this.carrots - cost);
        } else {
        ...
        // Sync to Firebase
        const change = this.carrots - oldCarrots;
